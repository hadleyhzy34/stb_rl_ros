...
class Env(gym.Env):
    def __init__(
        ...
        self.observation_space = spaces.Box(
            low=-np.inf, high=np.inf, shape=(362,), dtype=np.float32
        )
        self.goal_x = 0
        self.goal_y = 0
        ...
        self.sub_odom = rospy.Subscriber(
            self.namespace + "/odom", Odometry, self.getOdometry
        )
    ...
    def gc_collect_step(self, action):
        """
        Description: env step() function
        args:
            action: (2,)
    ...
    def gc_reset(self, seed=None, options=None):
    ...
    def random_pts_map(self):
        ...
        x1, x2, y1, y2 = 0, 0, 0, 0
        while (x1 - x2) ** 2 < 0.01:
            x1 = np.random.randint(5) + np.random.uniform(
                0.16, 1 - 0.16
            )  # random initialize x inside single map
            x2 = np.random.randint(5) + np.random.uniform(
                0.16, 1 - 0.16
            )  # random initialize goal position

        while (y1 - y2) ** 2 < 0.01:
            y1 = np.random.randint(5) + np.random.uniform(
                0.16, 1 - 0.16
            )  # random initialize y inside single map
            y2 = np.random.randint(5) + np.random.uniform(
                0.16, 1 - 0.16
            )  # random initialize goal position
